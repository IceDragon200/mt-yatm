...
function yatm_security.get_lockable_key_key(meta)
  assert(meta, "expected a meta")
  return yatm_security.lockable_key_schema:get_prvkey(meta)
end

function yatm_security.set_lockable_key_key(meta, prvkey)
  assert(meta, "expected a meta")
  yatm_security.lockable_key_schema:set_prvkey(meta, prvkey)
end

function yatm_security.copy_lockable_key_key(src_meta, dest_meta)
  assert(src_meta, "expected a source meta")
  assert(dest_meta, "expected a destination meta")
  local key = yatm_security.get_lockable_key_key(src_meta)
  yatm_security.set_lockable_key_key(dest_meta, key)
  return dest_meta
end

function yatm_security.get_lockable_object_key(meta)
  assert(meta, "expected a meta")
  return yatm_security.lockable_object_schema:get_pubkey(meta)
end

function yatm_security.set_lockable_object_key(meta, pubkey)
  yatm_security.lockable_object_schema:set_pubkey(meta, pubkey)
  return meta
end

function yatm_security.copy_lockable_object_key(src_meta, dest_meta)
  assert(src_meta, "expected a source meta")
  assert(dest_meta, "expected a destination meta")
  local key = yatm_security.get_lockable_object_key(src_meta)
  yatm_security.set_lockable_object_key(dest_meta, key)
end

function yatm_security.get_lockable_key_stack_key(stack)
  local meta = stack:get_meta()
  return yatm_security.get_lockable_key_key(meta)
end

function yatm_security.get_lockable_object_stack_key(stack)
  local meta = stack:get_meta()
  return yatm_security.get_lockable_object_key(meta)
end

function yatm_security.set_lockable_key_stack_key(stack, prvkey)
  local meta = stack:get_meta()
  yatm_security.set_lockable_key_key(meta, prvkey)
  return stack
end

function yatm_security.set_lockable_object_stack_key(stack, pubkey)
  local meta = stack:get_meta()
  yatm_security.set_lockable_object_key(meta, pubkey)
  return stack
end

function yatm_security.copy_lockable_key_stack_key(src, dest)
  local key = yatm_security.get_lockable_key_stack_key(src)
  yatm_security.set_lockable_key_stack_key(dest, key)
end
...
function yatm_security.gen_private_key()
...
function yatm_security.pair_lockables(key_stack, object_stack, prvkey)
  ...
  local prvkey = prvkey or yatm_security.gen_private_key()
  local pubkey = yatm_security.prvkey_to_pubkey(prvkey)
  yatm_security.set_lockable_key_stack_key(key_stack, prvkey)
  yatm_security.set_lockable_object_stack_key(object_stack, pubkey)
...
function yatm_security.is_stack_a_key_for_locked_stack(key_stack, lockable_stack)
  ...
  if yatm_security.is_stack_lockable_toothed_key(key_stack) then
    ...
    if yatm_security.is_stack_lockable_object(lockable_stack) then
      local prvkey = yatm_security.get_lockable_key_stack_key(key_stack)
      local pubkey = yatm_security.get_lockable_object_stack_key(key_stack)
...
function yatm_security.is_stack_a_key_for_locked_node(stack, pos)
  ...
  if yatm_security.is_stack_lockable_toothed_key(stack) then
    ...
    if yatm_security.item_is_lockable_object(lockable_nodedef) then
      local prvkey = yatm_security.get_lockable_key_stack_key(stack)

      local lockable_meta = minetest.get_meta(pos)
      local pubkey = yatm_security.get_lockable_object_key(lockable_meta)

...
function ic:update(cls, cluster, dtime)
  ...
  cluster:reduce_nodes_of_groups("thermal_producer", 0, function (node_entry, acc)
    ...
    if nodedef.thermal_interface then
      ...
      if heat_produced ~= 0 then
        for dir, vec3 in pairs(yatm_core.DIR6_TO_VEC3) do
          ...
          local neighbour_node = cluster:get_node(neighbour_pos)

          if neighbour_node and neighbour_node.thermal_interface then
            if neighbour_node.thermal_interface.update_heat then
              neighbour_node.thermal_interface:update_heat(neighbour_pos, neighbour_node, heat_produced, dtime)
            end
          end
        end
      end

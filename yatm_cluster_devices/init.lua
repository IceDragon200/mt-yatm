...
do
  ...
  function ic:get_node_infotext(pos)
    ...
    if cluster then
      ...
      if controller_id then
        ...
        comment = "no available controller"
      end

      return "Device Cluster: " .. cluster.id .. " (" .. state_string .. ") [" .. comment .. "]"
    end
  ...
  function ic:on_pre_block_expired(block)
    yatm.clusters:reduce_clusters_of_group(self.cluster_group, 0, function (cluster, acc)

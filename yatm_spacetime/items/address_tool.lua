      ...
      if Groups.get_item(nodedef, "addressable_spacetime_device") then
        ...
        if placer and placer:is_player() then
          if is_blank(address) then
            ...
            core.chat_send_player(placer:get_player_name(), "Blank Address!")
          else
            itemstack:get_meta():set_string("description", "Address Tool <" .. address .. ">")
            core.chat_send_player(placer:get_player_name(), "Address Copied!")
          end
        end
      end
    end
      ...
      if Groups.get_item(nodedef, "addressable_spacetime_device") then
        ...
        if user and user:is_player() then
          if is_blank(address) then
            ...
            core.chat_send_player(user:get_player_name(), "Address Pasted!")
          end
        end
      end
    end
...
core.register_tool("yatm_spacetime:address_tool", {

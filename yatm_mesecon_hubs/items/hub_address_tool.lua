      ...
      if Groups.get_item(nodedef, "addressable_hub_device") then
        ...
        if placer and placer:is_player() then
          if is_blank(address) then
            ...
            core.chat_send_player(placer:get_player_name(), "No Hub Address!")
          else
            itemstack:get_meta():set_string("description", "Hub Address Tool <" .. address .. ">")
            core.chat_send_player(placer:get_player_name(), "Hub Address Copied!")
          end
        end
      end
    end
    ...
    if nodedef then
      if Groups.get_item(nodedef, "addressable_hub_device") then
        ...
        if is_blank(new_address) then
          core.chat_send_player(user:get_player_name(), "No Address to paste!")
        else
          ...
          if user and user:is_player() then
            if is_blank(address) then
              core.chat_send_player(user:get_player_name(), "Hub Address Cleared!")
            else
              core.chat_send_player(user:get_player_name(), "Hub Address Pasted!")
            end
          end
        end
      end
...
core.register_tool("yatm_mesecon_hubs:hub_address_tool", {

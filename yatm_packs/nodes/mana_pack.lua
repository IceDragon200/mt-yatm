...
core.register_node("yatm_packs:mana_pack_empty", {
  ...
  on_rightclick = function (pos, node, clicker, itemstack, pointed_thing)
    if itemstack_has_group(itemstack, "mana_pack_pouch") then
      ...
      core.chat_send_player(clicker:get_player_name(), "The manapack is empty")
    end
  end,
})

core.register_node("yatm_packs:mana_pack", {
  ...
  on_rightclick = function (pos, node, clicker, itemstack, pointed_thing)
    ...
    if itemstack_has_group(itemstack, "mana_pack_pouch") then
      if charges_left < MAXIMUM_CHARGES then
        ...
        core.chat_send_player(clicker:get_player_name(), "The manapack is full")
      end
    else
      if charges_left > 0 then
        if recover_mp(clicker, MP_PER_CHARGE, "manapack") > 0 then
          ...
          core.chat_send_player(clicker:get_player_name(), "You are already at max mana")
        end
      else
        core.chat_send_player(clicker:get_player_name(), "The manapack is empty")
      end

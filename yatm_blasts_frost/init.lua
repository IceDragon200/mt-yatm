--[[
  ...
  FROST explosions only target nodes that are below air, instantly freezing them given enough
  strength.
    ...
    if Groups.has_group(freezable_nodedef, "freezable") then
      if freezable_nodedef then
        if freezable_nodedef.on_freeze then
          freezable_nodedef.on_freeze(pos, freezable_node, assigns.strength)

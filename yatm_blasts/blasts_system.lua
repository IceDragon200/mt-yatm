...
do
  ...
  function ic:init()
    --
    core.log("info", "attempting to reload explosions")

    if self.kv:marshall_load_file(self.m_filename) then
      --
      core.log("info", "loaded explosions")
    end
  ...
  function ic:_maybe_persist_explosions()
    if self.kv.dirty then
      ...
      core.mkdir(dirname)
      core.log("info", "persisting explosions")
      if self.kv:marshall_dump_file(self.m_filename) then
        self.kv.dirty = false
        core.log("info", "persisted explosions")
      else
        core.log("warning", "could not persist explosions")
      end
    end
  end
  ...
  function ic:create_explosion(pos, kind, params)
    ...
    core.log("error", "explosion type " .. kind .. " does not exist")

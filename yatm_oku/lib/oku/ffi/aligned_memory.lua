...
if not ffi then
  yatm.error("cannot create FFIAlignedMemory module, need ffi")
  return
end
...
--- @class FFIAlignedMemory
local FFIAlignedMemory = foundation.com.Class:extends("oku.FFIAlignedMemory")
do
  local ic = assert(FFIAlignedMemory.instance_class)
  ...
  function ic:check_bounds(index, len)
    len = len or 1
  ...
  function ic:adjust_and_check_bounds(local_index, size, len)
    len = len or 1
  ...
  for type_name, size in pairs({
    ...
    ic[type_name] = function (self, index)
  ...
  function ic:fill_slice(index, len, value)
    ffi.fill(self.m_data + index, len, value)
    return self
  end

  function ic:bytes(index, len)
    len = len or 1
    local result = {}
    local i = 1
  ...
  function ic:put_bytes(index, value)
...
yatm_oku.OKU.FFIAlignedMemory = assert(FFIAlignedMemory)

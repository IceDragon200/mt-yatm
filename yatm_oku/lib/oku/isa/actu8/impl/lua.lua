...
do
  ...
  function ic:step(memory)
    ...
    elseif ins == 0x10 then -- LDI <imm>
      self.pc = self.pc + 1
      local imm = memory:r_u8(self.pc)
      self.pc = self.pc + 1
      self.a = imm
      check_zero = true
    elseif ins == 0x11 then -- LDA <addr8>
      self.pc = self.pc + 1
      local addr8 = memory:r_u8(self.pc)
      self.pc = self.pc + 1
    ...
    elseif ins == 0x51 then -- CALL <addr16>
      if self.sp < 2 then
        self.flags.fault = 1
        self.fc = self.FAULT_STACK_OVERFLOW
      else
        local ret_pc = self.pc + 3
        local lo = ret_pc % 256
    ...
    elseif ins == 0x52 then -- RET
      if self.sp > 253 then
        self.flags.fault = 1
        self.fc = self.FAULT_STACK_UNDERFLOW
      else
  ...
  ::jump::
    do
      local lo = memory:r_u8(self.pc)
      self.pc = self.pc + 1
      local hi = memory:r_u8(self.pc)
      self.pc = self.pc + 1
  ...
  function ic:unpack_flags(flags)
    local zero = flags % 2
    flags = floor(flags / 2)
    local borrow = flags % 2
    flags = floor(flags / 2)
    local carry = flags % 2
    flags = floor(flags / 2)
    local interrupt = flags % 2
    flags = floor(flags / 2)
    local halt = flags % 2
    flags = floor(flags / 2)
    local fault = flags % 2

...
do
  ...
  function ic:check_network_member(member, network)
    local node = core.get_node(assert(member.pos))
  ...
  function ic:get_member(pos)
    local id = core.hash_node_position(pos)
  ...
  function ic:update_member(pos, node, is_register)
    ...
    if is_register then
      print(self.m_description, "update_member/3", "registering", core.pos_to_string(pos), node_to_string(node))
    else
      print(self.m_description, "update_member/3", "updating registration", core.pos_to_string(pos), node_to_string(node))
    end
    ...
    if old_record then
      if is_register then
        print(self.m_description, "WARN", "duplicate registration attempted", core.pos_to_string(pos), node.name)
      else
        print(self.m_description, "removing old record", core.pos_to_string(pos), node.name)
      end
      ...
      if old_record.network_id then
        ...
        if self.m_networks[old_network_id] then
          ...
          print(self.m_description, "ERROR", "network does not exist", old_network_id, core.pos_to_string(pos), node.name)
        end
      end
    end
  ...
  function ic:queue_all_adjacent(pos)
    ...
    for dir,v3 in pairs(Directions.DIR6_TO_VEC3) do
      ...
      neighbour_node_id = core.hash_node_position(neighbour_pos)
  ...
  function ic:unregister_member(pos)
    ...
    if record then
      ...
      print(self.m_description, "unregister_member/1", "nothing was registered at that position!?", core.pos_to_string(pos))
    end
  end
  ...
  function ic:get_connected_pos(pos, node, nodedef, to_visit)
    ...
    for _d6,v3 in pairs(Directions.DIR6_TO_VEC3) do
      ...
      local neighbour_node = core.get_node(npos)
      ...
      for network_id,_ in pairs(old_invalid_networks) do
        ...
        if network then
          for member_hash,member_entry in pairs(network.members) do
            node = core.get_node(member_entry.pos)
      ...
      if should_continue then
        ...
        while next(to_visit) do
          ...
          for _,opos in ipairs(old_to_visit) do
            local ohash = core.hash_node_position(opos)
            ...
            if entry then
              ...
              if entry.network_id then
                ...
                if n then
                  ...
                  print(self.m_description, "WARN", "node still exists in a network", core.pos_to_string(entry.pos), entry.network_id)
              ...
              local meta = core.get_meta(entry.pos)

...
if nokore_stairs then
  core.log("info", "using nokore_stairs.build_nodes/2 for decor nodes")
  yatm.build_decor_nodes = nokore_stairs.build_nodes
else
  core.log("warning", "using yatm.build_decor_nodes/2 for decor nodes, this is a no-op function")
...
function yatm.register_decor_nodes(basename, data)
  ...
  for suffix, node_def in pairs(parts) do
    core.register_node(basename .. "_" .. suffix, node_def)
  end
end
